...
public class ILMapper {
    private readonly static Dictionary<(Type, Type), MethodInfo> _cache = [];
    public static TDestination Map<TDestination>(object source) {
        ...
        if (!_cache.TryGetValue(key, out var methodInfo)) {
            _cache[key] = methodInfo = CreateMapperMethod(sourceType, destinationType);
        }

        return (TDestination)methodInfo.Invoke(null, [source])!;
    }

    static MethodInfo CreateMapperMethod(Type sourceType, Type destinationType) {
